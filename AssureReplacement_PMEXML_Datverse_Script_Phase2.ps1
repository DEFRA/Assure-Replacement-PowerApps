...
{ 
  ...
  #  $PMEsharedpath = $PMEFilesDownloadLocation
   ...
   try
   {
    ...
    $Destinationsharedpath = Get-Item -Path $PMEFilesDownloadLocation -ErrorAction Stop

     $Destinationsharedpath.GetAccessControl()
    ...
    "======================================================================================================================" | Out-File $ErrorLogFile -Append
     "===== Unable to access $PMEFilesDownloadLocation. Aborting this download process. ========" | Out-File $ErrorLogFile -Append
    "=======$EnvironmentName==PME Script End ==============="+(Get-Date).ToString("dd/MM/yyyy HH:mm:ss") +"==============" | Out-File $ErrorLogFile -Append
        "===== Exception details. ========" | Out-File $ErrorLogFile -Append
 ...
 if (Test-Path -Path  $PMEFilesDownloadLocation -ErrorAction Stop) 
  
{
  ...
  #   if (Test-Path -Path  $PMEFilesDownloadLocation) 
...
for ($i=0; $i -lt $getApiResponseObject.value.Length; $i++)
{
  ...
  #Build xml file URL to download
   ...
   # $destination = ( New-Item -Path "$PMEFilesDownloadLocation\$xmlfilename" -Force ) 
